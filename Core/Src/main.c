...
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *_hcan)
{
  if (_hcan == &hcan)
	{
		MotorControl_ProcessCanMessage();
	}
  else return;
}
...
/*	for (uint8_t i = 0; i < NUMBER_OF_MOTORS; i++) {
		MotorControl_SetMotorVelocity(MOTOR_CAN_IDS[i], 500);
	}
*/
  ...
  while (1)
  {
    ...
    // Her loop'ta motorların pozisyonu mutlaka güncellenmeli
	    Testtime = HAL_GetTick();
	  	MotorControl_RequestMessage();
	 	for (uint8_t i = 0; i < NUMBER_OF_MOTORS; i++)
		{
			MotorControl_ReadPosition(MOTOR_CAN_IDS[i]);
		}
		for (uint8_t i = 0; i < 3; i++)
		{
			for (uint8_t i = 0; i < NUMBER_OF_MOTORS; i++)
			{
				MotorControl_ProcessMessageBuffer(MOTOR_CAN_IDS[i]);
				HAL_Delay(1);
			}
		}
		...
		periodTime = HAL_GetTick() - Testtime;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : PA0 nFault_CANBUS_Pin */
  GPIO_InitStruct.Pin = GPIO_PIN_0|nFault_CANBUS_Pin;
